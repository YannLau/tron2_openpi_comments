"""Replay TRON2 recorded actions from a parquet file.

This script is intentionally independent from the policy server. It reads a
recorded trajectory and sends the recorded joint/gripper targets directly to
the robot through ``tron2_env``.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--deploy-config",
        default="config/tron2_deploy.local.yaml",
    ...
    parser.add_argument("--dry-run", action="store_true", help="Load and validate the file without moving the robot.")
    return parser.parse_args()


def _section(profile: dict[str, Any], name: str) -> dict[str, Any]:
    return _deploy_config.section(profile, name)


def _robot_config(profile: dict[str, Any], robot_ip: str | None) -> Tron2Config:
...
def main() -> None:
    args = _parse_args()
    profile = _deploy_config.load_deploy_config(args.deploy_config)
    ...
    control_backend = str(client_profile.get("control_backend", robot_profile.get("control_backend", "websocket")))
    ...
    if args.dry_run:
        for step in range(start_step, min(end_step, start_step + 3)):
            ...
            print(f"step={step} action_shape={action.shape} arm_head={_full_servo_action(action, np.array(DEFAULT_INIT_HEAD)).shape}")
    ...
    with create_motion_controller(
        ...
        for step in range(start_step, end_step):
            ...
            gripper = np.clip(np.array([action[7], action[15]], dtype=np.float64) * 100.0, 0.0, 100.0)
