...
新增硬约束：**完整利用官方 π0.5 基座权重，保留原模型参数路径与形状，新增模块独立初始化。** 结构扩展不等于旧权重不可用，但当前加载器需增加严格的新增模块白名单，部署也需保存完整增强 checkpoint。详细的加载机制、冻结策略和能力保留验收见 [DSE_PI05_PRETRAINED_COMPATIBILITY.md](DSE_PI05_PRETRAINED_COMPATIBILITY.md)。能加载全部原权重、冻结原参数、保留泛化行为是三个不同要求，不能相互替代。
