...
class Policy(BasePolicy):
        ...
        *,
        ...
        if self._is_pytorch_model:
            ...
            self._sample_actions = nnx_utils.module_jit(
                model.sample_actions, static_argnames=("trained_rtc_mode",)
            )
    ...
    def infer(
        ...
        *,
        ...
        # Training-time RTC inference: use prefix conditioning without VJP
        ...
        if prev_chunk_left_over is not None:
            if self._is_pytorch_model:
                prev_chunk_left_over = torch.from_numpy(np.array(prev_chunk_left_over)).to(self._pytorch_device)
        ...
        if prev_chunk_left_over_len is not None and not self._is_pytorch_model:
            sample_kwargs["prev_chunk_left_over_len"] = jnp.array(prev_chunk_left_over_len)
        if prefix_horizon is not None:
            if self._is_pytorch_model:
                sample_kwargs["prefix_horizon"] = prefix_horizon
            else:
                sample_kwargs["prefix_horizon"] = jnp.array(prefix_horizon)
        ...
        # Capture raw actions (in normalized/model space) before output_transform.
        # Save separately to avoid corruption by jax.tree.map batch-dim removal
        # and to survive output_transforms that may drop extra keys.
        saved_raw_actions = None
        if return_raw_actions:
            if self._is_pytorch_model:
                saved_raw_actions = np.asarray(outputs["actions"][0, ...].detach().cpu())
            else:
                saved_raw_actions = np.asarray(outputs["actions"][0, ...])
        ...
        # Restore raw_actions after output_transform (transforms may drop extra keys)
        if saved_raw_actions is not None:
            outputs["raw_actions"] = saved_raw_actions
